 ...
 */
class InstanceManager extends events_1.EventEmitter {
    constructor(nodecg, services, bundles) {
        super();
        this.nodecg = nodecg;
        ...
        bundles.on("reregisterInstance", (serviceInstance) => this.reregisterHandlersOfInstance(serviceInstance));
    }
    /**
     ...
     */
    getServiceInstance(instanceName) {
     ...
     */
    createServiceInstance(serviceType, instanceName) {
        if (!instanceName) {
     ...
     */
    updateInstanceConfig(instanceName, config, validation = true) {
     ...
     */
    async updateInstanceClient(inst, instanceName, service) {
     ...
     */
    reregisterHandlersOfInstance(instanceName) {
        if (!instanceName)
            return;
        ...
        if (!svc.result.removeHandlers)
            return; // Service provides no way to remove handlers, thus this service has no handlers
        // Remove handlers
        try {
