...
class NanoleafUtils {
     ...
     */
    static async verifyIpAddress(ipAddress) {
     ...
     */
    static async verifyAuthKey(ipAddress, authToken) {
        const code = await this.checkConnection(ipAddress, authToken);
        return code === 200;
    }
    static buildBaseRequestAddress(ipAddress, authToken) {
     ...
     */
    static async retrieveAuthKey(ipAddress, nodecg) {
        ...
        try {
            const response = await (0, node_fetch_1.default)(`http://${ipAddress}:${this.defaultPort}/api/v1/new`, { method: "POST" });
    ...
    static async checkConnection(ipAddress, authToken) {
        try {
            const response = await (0, node_fetch_1.default)(NanoleafUtils.buildBaseRequestAddress(ipAddress, authToken), {
                method: "GET",
            });
